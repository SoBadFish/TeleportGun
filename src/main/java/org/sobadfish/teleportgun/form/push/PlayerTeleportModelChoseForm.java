...
public class PlayerTeleportModelChoseForm extends CustomButtonForm {
    ...
    public void callback(FormResponseSimple response) {
       if(response.getClickedButtonId() == 0){
        ...
        if(response.getClickedButtonId() == 1){
            PlayerOpenPrivatePointForm playerOpenPointForm = new PlayerOpenPrivatePointForm("传送枪配置-私有传送点","",playerInfo,item);
            TeleportGunMainClass.INSTANCE.formManager.addForm(playerInfo,playerOpenPointForm);
        }
    ...
    public void onCreateView() {
        ...
        addButton(new ElementButton("设置私有传送点",new ElementButtonImageData("path","textures/particle/teleport_door")));
